...
import { setSelectedDay } from "../store/task-slice";
...
export class MainSection extends Component {
  ...
  private handleDayChange(event: Event): void {
    ...
    store.dispatch(setSelectedDay(selectedDay));
    this.update();
  }
  ...
  render(): string {
    ...
    const html = /*html*/ `
      <section class="bg-main-pattern bg-no-repeat bg-cover h-screen flex items-center justify-center px-4 py-10 md:px-2">
        <div class="bg-white border-4 border-black p-4 w-full max-w-3xl">
          <div class="flex flex-col gap-2">
            <div class="flex justify-between items-baseline gap-2">
              <h3 class="font-archivo font-medium max-w-48 sm:max-w-full truncate" id="selected-day">${extendedDay}</h3>
              <button id="addButton" class="font-archivo text-gray-500 font-light tracking-widest" aria-label="Add new task">Add</button>
            </div>
            <form class="flex flex-col gap-2" onsubmit="return false;" role="search">
              <div class="flex gap-2">
                <label for="underline_category" class="sr-only">By category</label>
                <select id="underline_category" aria-label="Select task category" class="block py-2.5 px-0 w-full text-sm text-gray-500 bg-transparent border-0 border-b-2 border-gray-200 appearance-none dark:text-gray-400 dark:border-gray-700 focus:outline-none focus:ring-0 focus:border-gray-200 peer">
                  ${uniqueCategories
                    .map(
                      (category) => /*html*/ `
              ...
              <div class="relative w-full">
                ...
                ${
                  this.tempSearchQuery
                    ? /*html*/ `
                    <button id="clear-search" type="button" class="absolute inset-y-0 end-0 pe-3 flex items-center" aria-label="Clear search">
                      <img src="/icons/x.svg" alt="Clear search icon"/>
                    </button>`
                    : ""
                }
              </div>
          ...
          <ul class="mt-2 p-px h-96 overflow-y-scroll space-y-2 tasks-container" role="list">
            ${
